package microservices.simulations;

import io.gatling.javaapi.core.*;
import io.gatling.javaapi.http.HttpProtocolBuilder;
import microservices.consumes.ObtenerScenario;
import microservices.consumes.ParticipantesScenario;
import microservices.consumes.URLRequestScenario;
import microservices.consumes.VersionRequestScenario;

import static io.gatling.javaapi.core.CoreDsl.*;
import static io.gatling.javaapi.http.HttpDsl.*;

public class CelulaTestplan extends Simulation{
    int nbUser = Integer.getInteger("users", 1);
    int myRamp = Integer.getInteger("ramp", 0);

    String baseURL = System.getProperty("baseURL", "localhost");

    HttpProtocolBuilder httpProtocol = http
            .baseUrl(baseURL)
            .header("Content-type","application/json; charset=utf-8")
            .check(status().is(200));
    {
        setUp(
                URLRequestScenario.GetCelulaPage().injectOpen(rampUsers(nbUser).during(myRamp)),
                VersionRequestScenario.GetVersion().injectOpen(rampUsers(nbUser).during(myRamp)),
                ParticipantesScenario.GetPArticipants().injectOpen(rampUsers(nbUser).during(myRamp)),
                ObtenerScenario.GetSomething().injectOpen(rampUsers(nbUser).during(myRamp))
        ).protocols(httpProtocol);
    }
}
